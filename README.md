...
- **主程序四个页面**：今日（完成率进度条 + 待办/已完成分组 + 日期切换 + 今日事明日毕）、本周（一周概览条 + 按天分组，任务可按住拖到别的日期，拖到列表上/下边缘会自动翻页）、已完成（按完成日期归档、可撤销、可清理）、设置（插件层级/透明度/大小、主题色、全屏自动隐藏、开机自启）。
...
| 标签 | 版本 | 说明 |
| --- | --- | --- |
| `v1.7` | 1.7 | **特别版（当前）**：圣旨皮肤 + 主程序与插件都有小人；新增「今日事明日毕」，修好插件右键「打开主程序」 |
